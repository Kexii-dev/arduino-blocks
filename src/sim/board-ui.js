...
export class BoardUI {
  constructor(board, hooks = {}) {
    ...
    this.buttons = {}; // pin -> SVG group (pins 2 & 3)
  ...
  build(container) {
    ...
    const svg = el('svg', { viewBox: '0 0 820 700', class: 'sim-board-svg', xmlns: 'http://www.w3.org/2000/svg' });
    this._svgEl = svg;
    ...
    // ---- Zone de contrôle (sous la carte, y > 600) ----------------------
    const ctrlY = 610;
    svg.appendChild(el('rect', { x: 10, y: ctrlY, width: 800, height: 82, rx: 10, fill: '#16181a', stroke: '#2a2d30' }));
    svg.appendChild(el('text', { x: 20, y: ctrlY + 16, fill: '#9fd8db', 'font-size': 10, 'font-family': 'monospace' }, ['CONTRÔLES']));

    // Boutons D2 & D3 (impulsion)
    let bx = 26;
    for (const p of [2, 3]) {
      const g = el('g', { transform: 'translate(' + bx + ',' + (ctrlY + 34) + ')', 'data-pin': p, class: 'sim-btn' });
      g.appendChild(el('rect', { x: 0, y: 0, width: 60, height: 32, rx: 6, fill: '#c0392b', stroke: '#7d2418', 'stroke-width': 1.5 }));
      g.appendChild(el('rect', { x: 6, y: 6, width: 48, height: 20, rx: 4, fill: '#a93226' }));
      g.appendChild(el('text', { x: 30, y: 46, fill: '#eee', 'font-size': 9, 'font-family': 'monospace', 'text-anchor': 'middle' }, ['Btn D' + p]));
      this.buttons[p] = g;
      svg.appendChild(g);
      bx += 96;
    }

    // Sliders analogiques A0..A5
    const sx = bx + 14;
    svg.appendChild(el('text', { x: sx, y: ctrlY + 16, fill: '#9fd8db', 'font-size': 9, 'font-family': 'monospace' }, ['ANALOG']));
    for (let c = 0; c < 6; c++) {
      const x = sx + 90 + c * 42;
      const trace = el('line', { x1: x, y1: ctrlY + 30, x2: x, y2: ctrlY + 60, stroke: '#3a8b91', 'stroke-width': 5, 'stroke-linecap': 'round' });
      const knob = el('circle', { cx: x, cy: ctrlY + 30, r: 6, fill: teal, stroke: '#dff6f7', 'stroke-width': 1.5, 'data-ch': c, class: 'sim-knob' });
      const label = el('text', { x: x, y: ctrlY + 78, fill: '#cfe', 'font-size': 8, 'font-family': 'monospace', 'text-anchor': 'middle', 'data-ch': c }, ['A' + c]);
      this.sliders[c] = { trace, knob, label, x, minY: ctrlY + 30, maxY: ctrlY + 60, val: 0 };
      svg.appendChild(trace); svg.appendChild(knob); svg.appendChild(label);
    }

    // Buzzer (pin 11), à droite
    const buz = el('g', { transform: 'translate(760,' + (ctrlY + 42) + ')' });
    ...
    svg.appendChild(buz);
    svg.appendChild(el('text', { x: 760, y: ctrlY + 74, fill: '#9fd8db', 'font-size': 8, 'font-family': 'monospace', 'text-anchor': 'middle' }, ['Buzzer D11']));
