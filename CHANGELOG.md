...
### Ajouté
- **Carte Arduino Mega** : sélecteur de carte dans la barre de compile (Arduino Uno / Arduino Mega). Le choix persiste (localStorage) ; le titre du panneau C++ et le message de compile suivent la carte. Compile avec le bon FQBN (`arduino:avr:mega`), flash Avrgirl `mega`. Backend : whitelist `uno`/`mega`, rejette toute carte inconnue (400).
